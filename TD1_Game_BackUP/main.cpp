							...
							#pragma region エフェクトとの衝突判定
														for (int j = 0; j < afterimageMax; j++) {
															...
															if (afterimage[j].carentAlpha == 0xA0 && afterimage[j].isActive == true && isAttacking == true) {
																...
																ef2e[i].distance.x = afterimage[j].translate.x - enemy[i].translate.x;
																ef2e[i].distance.y = afterimage[j].translate.y - enemy[i].translate.y;

																ef2e[i].length = sqrt(pow(ef2e[j].distance.x, 2) + pow(ef2e[j].distance.x, 2));

																if (ef2e[i].length <= afterimage[j].radius / 2 + enemy[i].radius / 2) {
